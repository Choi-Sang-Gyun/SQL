--DML문
--INSERT
DESC DEPARTMENTS; --데이터의 구조를 빠르게 확인
--1ST (컬럼을 정확히 일치 시키는 경우)
...
--2ND
INSERT INTO EMPS(EMPLOYEE_ID, LAST_NAME, EMAIL, HIRE_DATE, JOB_ID)
VALUES( --서브쿼리, 값, 값, 값, 값 같은 식으로 들어가는 것도 가능하다
(SELECT EMPLOYEE_ID FROM EMPLOYEES WHERE FIRST_NAME = 'Lex'), 'EXAMPLE', 'EXAMPLE', SYSDATE, 'EXAMPLE' );
...
--JOB_ID가 같은 사람이 4명이라 4명이 삭제가 됨
-- (SELECT JOB_ID FROM EMPS WHERE EMPLOYEE_ID = 121) -> 121의 JOB_ID를 구하는 값

--모든 데이터가 전부 지워질 수 있는 것은 아님
SELECT * FROM DEPARTMENTS;
SELECT * FROM EMPLOYEES;
--50번 부서는 EMPLOYEES테이블에서 참조되고 있기 때문에 삭제가 일어나면 참조무결성 제약을 위배됩니다. 그래서 삭제가 안됨!
...
--2ND -MERGE문으로 직접 특정 데이터에 값을 넣고자 할때 사용할 수 있음
MERGE INTO EMPS E1
USING DUAL
...
INSERT INTO DEPTS(SELECT * FROM DEPARTMENTS);
...
MERGE INTO OBS_IT O
USING (SELECT * FROM JOBS_IT) J
ON (J.MIN_SALARY > 0)
WHEN MATCHED THEN
     UPDATE SET O.MIN_SALARY = J.MIN_SALARY,
                O.MAX_SALARY = J.MAX_SALARY
WHEN NOT MATCHED THEN
     INSERT VALUES (O.MIN_SALARY, O.MAX_SALARY);
