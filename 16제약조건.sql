--제약조건 - 컬럼에 원치 않는 데이터가 입력, 삭제, 수정되는 것을 방지하기 위한 조건
--PRIMARY KE - 테이블의 고유키, 중복X, NULL X, PK는 테이블에 1개
--UNIQUE - 중복X, NULL O,
...
CREATE TABLE DEPTS(
    ...
    DEPT_DATE DATE          DEFAULT SYSDATE, --DEFAULT (값) -> 값이 들어가지 않을 때 자동으로 지정되는 값 / 제약 조건은 아님
    ...
    LOCA_ID NUMBER(4)       CONSTRAINT DEPTS_LOCA_ID_FK REFERENCES LOCATIONS(LOCATION_ID) --REFERENCES 참조테이블(컬럼명)
);
...
CREATE TABLE DEPTS(
    ...
    LOCA_ID NUMBER(4)       REFERENCES LOCATIONS(LOCATION_ID) --REFERENCES 참조테이블(컬럼명)
);
DESC DEPTS;
...
INSERT INTO DEPTS(DEPT_NO, DEPT_NAME, DEPT_PHONE, DEPT_GENDER, LOCA_ID)
VALUES(1, 'HONG', '010....', 'F', 100); --FK 참조제약 위배

INSERT INTO DEPTS(DEPT_NO, DEPT_NAME, DEPT_PHONE, DEPT_GENDER, LOCA_ID)
VALUES(1, 'HONG', '010....', 'F', 1700);

INSERT INTO DEPTS(DEPT_NO, DEPT_NAME, DEPT_PHONE, DEPT_GENDER, LOCA_ID)
VALUES(2, 'HONG', '010....', 'F', 1700); --유니크 제약 위배(폰번호)


--개체 무결성 : 기본 키에 NOT NULL일 수 없고, 중복 될 수 없다 규칙
--참조 무결성 : 참조하는 테이블의 PK만 FK컬럼에 들어갈 수 있다 규칙
--도메인 무결성 : CHECK, UNIQUE 제약을 위배할 수 없다 규칙
------------------------------------------------------------------------------
--2ND - 테이블 레벨 제약조건
DROP TABLE DEPTS;
...
CREATE TABLE DEPTS (
        ...
        CONSTRAINT DEPTS_LOCA_KF FOREIGN KEY(LOCA_ID) REFERENCES LOCATIONS(LOCATION_ID)
...
ALTER TABLE DEPTS ADD CONSTRAINT DEPT_GENGER_CK CHECK(DEPT_GENDER IN('F', 'M'));
...
ALTER TABLE DEPTS DROP CONSTRAINT DEPT_LOCA_ID_FK /* 제약 조건 이름 */ ; --이름으로도 지울 수 있음
--테이블의 제약조건을 SQL문으로 확인
SELECT * FROM USER CONSTARINT WHERE TABLE_NAME = 'DEPTS';
...
CREATE TABLE MEMS (
        ...
        SALARY NUMBER(10)   CONSTRAINT MEM_GENDER_CK CHECK(SALARY > 0),
        LOCA NUMBER(4)      CONSTRAINT MEM_LOCA_LOC_LOCID_FK REFERENCES LOCATIONS(LOCATION_ID)  
);

ALTER TABLE MEMS ADD CONSTRAINT MEM_REGDATE_UK UNIQUE(REG_DATE);

INSERT INTO MEMS VALUES ('AAA', 1, '18/07/01', 5000 , 1800);
INSERT INTO MEMS VALUES ('BBB', 2, '18/07/02', 3000, 1900);
INSERT INTO MEMS VALUES ('CCC', 3, '18/07/03', 3000, 2000);
INSERT INTO MEMS VALUES ('DDD', 4, SYSDATE, 2000, 2000);
...
CREATE TABLE BOOKLISTS (
    BOOK_NO VARCHAR(10)         CONSTRAINT BOOK_NO_PK PRIMARY KEY,
...
CREATE TABLE BOOKRENTS (
    RENT_NO NUMBER(6)           CONSTRAINT RENT_NO_PK PRIMARY KEY,
    BOOK_NO VARCHAR(10)         CONSTRAINT BOOK_NO_RENT_KF REFERENCES BOOKLISTS(BOOK_NO),
    RENT_DATE DATE              NOT NULL,
    RETURN_DATE DATE            NOT NULL,
    RIGHT_RETURN CHAR(1)        CONSTRAINT RIGHT_RETURN_CK CHECK(RIGHT_RETURN IN ('Y','N'))
);

DROP TABLE BOOKRENTS;
