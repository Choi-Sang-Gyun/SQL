...
--강제 형변환
--TO CHAR  1)날짜 -> 문자 강제형변환(날짜 포멧형식 쓰임)
SELECT SYSDATE, TO_CHAR( SYSDATE, 'YYYY-MM-DD HH:MI:SS') FROM DUAL;
SELECT SYSDATE, TO_CHAR(SYSDATE, 'YYYY"년" MM"월" DD"일"') FROM DUAL; --데이트 포맷 형식이 아닌 값은 ""로 묶으면 됩니다
SELECT FIRST_NAME, TO_CHAR(HIRE_DATE, 'YY-MM-DD AM') FROM EMPLOYEES;

--TO_CHAR = 숫자 -> 문자로 강제 형변환 (숫자 포멧형식 쓰입니다)
SELECT TO_CHAR (20000, '9999999999999')  FROM DUAL; --9는 자리수
SELECT TO_CHAR (20000, '0999999999999')  FROM DUAL; --남는 자리는 0으로 채움
SELECT TO_CHAR (20000, '9999')  FROM DUAL; --자리가 부족하면 #으로 처리됨
SELECT TO_CHAR (20000.123, '99999.999') FROM DUAL;
SELECT TO_CHAR (20000, 'L999,999,999.99') FROM DUAL; --$는 그냥 쓰면 됩니다. L을 붙이면 지역 화폐기호

SELECT '2000' + 2000 FROM DUAL; -- 자동형변환
--TO_NUMBER = 문자 -> 숫자 강제 형변환
SELECT '$2,000' + 2000 FROM DUAL; -- 불가능
SELECT TO_NUMBER('$2,000', '$9,999') + 2000 FROM DUAL; -- 문자를 강제로 숫자로 바꾼 뒤 계산

--TO_DATE = 문자 -> 날짜 로 강제 형변환
...
SELECT DECODE('B', 'A', 'A입니다', 'B', 'B입니다', 'C', 'C입니다', '나머지 입니다') FROM DUAL;
...
SELECT COALESCE(NULL, NULL, NULL) FROM DUAL; --값이 다 NULL이면 반환값도 NULL
...
SELECT SALARY, TO_CHAR (SALARY*1302.69, 'L999,999,999.99') FROM EMPLOYEES;
--문자에서 문자로 바로 못 바꾸니, 문자->날짜->문자 식으로 변환할 것
...
CASE WHEN FLOOR(SYSDATE - HIRE_DATE) > 3650 THEN FLOOR(SYSDATE - HIRE_DATE)
     ELSE NULL
     END
     FROM EMPLOYEES ORDER BY FLOOR(SYSDATE - HIRE_DATE) DESC ; 

--SELECT EMPLOYEE_ID AS 사원번호,
--       CONCAT(FIRST_ID, LAST_ID) AS 사원이름,
...
--문제 3.
--EMPLOYEE 테이블의 manager_id컬럼을 확인하여 
--first_name, manager_id, 직급을 출력합니다.
--100이라면 ‘부장’ 
--120이라면 ‘과장’
--121이라면 ‘대리’
--122라면 ‘주임’
...
DECODE(MANAGER_ID, 100, '부장', 120, '과장', 121, '대리', 122, '주임', '사원')
...
SELECT CONCAT(FIRST_NAME, LAST_NAME) AS 이름,
       ...
       TO_CHAR((SALARY + SALARY* NVL(COMMISSION_PCT, 0))*1300, 'L999,999,999.99') AS 급여,
