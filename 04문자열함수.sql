--문자열 함수
SELECT 'abcDER', LOWER('abcDEF'), UPPER('abcDEF'), INITCAP('abcDEF') FROM DUAL;
...
--SUBSTR(타겟, 시작위치, 자를 문자갯수) -  문자열 자르기, CONCAT - 문자열 붙이기
SELECT FIRST_NAME, SUBSTR(FIRST_NAME, 1, 3) FROM EMPLOYEES;
SELECT CONCAT(FIRST_NAME, LAST_NAME), FIRST_NAME || LAST_NAME FROM EMPLOYEES;

--LPAD -왼쪽 공백을 특정 값으로 채움 (가운데 숫자는 오른쪽 걸 그 숫자만큼 쓴다는 게 아니라 본래의 수치+ 오른쪽 = 가운데 숫자 의 개념이다 )
SELECT LPAD(FIRST_NAME, 10, '*') FROM EMPLOYEES;
--LPAD -오른쪽 공백을 특정 값으로 채움 (가운데 숫자는 오른쪽 걸 그 숫자만큼 쓴다는 게 아니라 본래의 수치+오른쪽 = 가운데 숫자 의 개념이다)
SELECT RPAD(FIRST_NAME, 10, '*') FROM EMPLOYEES;

--TRIM -양쪽 공백제거, LTRIM -왼쪽에서 제거, RTRIM -오른쪽에서 제거
SELECT TRIM('    HELLO WORLD ') FROM DUAL;
SELECT LTRIM('    HELLO WORLD ') FROM DUAL; --더블클릭 해보면 공백 남아있다는 걸 확인 가능
SELECT RTRIM('    HELLO WORLD ') FROM DUAL;
--매개변수를 두개 주면 오른쪽 애를 왼쪽에서 찾아서 제거
...
SELECT '(02)' || SUBSTR(PHONE_NUMBER, 4, (LENGTH(PHONE_NUMBER)-3)) FROM EMPLOYEES;
